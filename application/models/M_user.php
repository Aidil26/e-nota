...
class M_user extends CI_Model 
{
	function validasi_login($username, $password)
	{
		return $this->db
			...
			->join('props_akses b', 'a.id_akses = b.id_akses', 'left')
	...
	function fetch_data_user($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			...
			FROM 
				`props_user` AS a 
				LEFT JOIN `props_akses` AS b ON a.`id_akses` = b.`id_akses` 
	...
	function tambah_baru($username, $password, $nama, $id_akses, $status)
	{
		...
		return $this->db->insert('props_user', $dt);
	}
	...
	function get_baris($id_user)
	{
		$sql = "
			...
			FROM 
				`props_user` a 
				LEFT JOIN `props_akses` b ON a.`id_akses` = b.`id_akses` 
			WHERE 
				a.`id_user` = '".$id_user."' 
			LIMIT 1
		";
