...
class M_barang extends CI_Model 
{
	function fetch_data_barang($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			...
			FROM 
				`props_barang` AS a 
				LEFT JOIN `props_kategori_barang` AS b ON a.`id_kategori_barang` = b.`id_kategori_barang` 
				LEFT JOIN `props_merk_barang` AS c ON a.`id_merk_barang` = c.`id_merk_barang` 
	...
	function tambah_baru($kode, $nama, $id_kategori_barang, $id_merk_barang, $stok, $harga, $keterangan)
	{
		...
		return $this->db->insert('props_barang', $dt);
	}
	...
	function update_barang($id_barang, $kode_barang, $nama, $stok, $harga, $id_kategori_barang, $id_merk_barang, $keterangan)
	{
		$dt = array(
	...
	{
		$sql = "
			UPDATE `props_barang` SET `total_stok` = `total_stok` - ".$jumlah_beli." WHERE `id_barang` = '".$id_barang."'
		";
