...
{
	function show_toko(){
		$toko = "SELECT * FROM props_toko WHERE id_toko = 1";
		return $this->db->query($toko);
	}
	...
	function insert_master($nomor_nota, $tanggal, $id_kasir, $id_pelanggan, $bayar, $grand_total, $catatan)
	{
		...
		return $this->db->insert('props_penjualan_master', $dt);
	}
	...
	function fetch_data_penjualan($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			...
			FROM 
				`props_penjualan_master` AS a 
				LEFT JOIN `props_pelanggan` AS b ON a.`id_pelanggan` = b.`id_pelanggan` 
				LEFT JOIN `props_user` AS c ON a.`id_user` = c.`id_user` 
	...
	function hapus_transaksi($id_penjualan, $reverse_stok)
	{
		if($reverse_stok == 'yes'){
			$loop = $this->db
				...
				->get('props_penjualan_detail');

			foreach($loop->result() as $b)
			{
				$sql = "
					UPDATE `props_barang` SET `total_stok` = `total_stok` + ".$b->jumlah_beli." 
		...
		$this->db->where('id_penjualan_m', $id_penjualan)->delete('props_penjualan_detail');
		return $this->db
			->where('id_penjualan_m', $id_penjualan)
			->delete('props_penjualan_master');
	}
	...
	{
		return $this->db->select('nomor_nota')->where('nomor_nota', $nota)->limit(1)->get('props_penjualan_master');
	}
