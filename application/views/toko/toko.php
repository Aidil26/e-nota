...
<body class="hold-transition skin-purple-light sidebar-mini fixed sidebar-collapse">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="box box-solid bg-gray">
					<div class="box-header with-border"><h5><i class='fa fa-users fa-fw'></i> Daftar Toko</h5></div>
	...
	<?php
	$tambahan = nbs(2)."<a href='".site_url('toko/add_toko')."' class='btn btn-default' id='TambahToko'><i class='fa fa-plus fa-fw'></i> Tambah Toko</a>";
	...
	"ajax":{
	url :"<?php echo site_url('toko/toko_json'); ?>",
	type: "post",
	error: function(){
	$(".my-grid-error").html("");
	...
	$(document).on('click', '#TambahToko, #EditToko', function(e){
	e.preventDefault();
	if($(this).attr('id') == 'TambahToko')
	{
	$('.modal-dialog').removeClass('modal-lg');
	$('.modal-dialog').removeClass('modal-sm');
	$('#ModalHeader').html('Tambah Toko');
	}
	if($(this).attr('id') == 'EditToko')
	{
	$('.modal-dialog').removeClass('modal-lg');
	$('.modal-dialog').removeClass('modal-sm');
	$('#ModalHeader').html('Edit Toko');
	}
